...
export default function PullToRefresh({
  ...
  const iconPosition =
    pullDistance > 0 || refreshing ? initialIconOffset + pullDistance : -50;

  return (
    <div
      className={`relative min-h-full flex flex-col ${backgroundColor} overflow-hidden`}
    >
      {/* Moving refresh icon */}
      <div
          ...
          <RefreshCw
            ...
            style={{
              transform: refreshing
                ? "rotate(0deg)"
                : `rotate(${-1 * (pullDistance / refreshThreshold) * 360}deg)`,
            }}
          />
        </div>
      </div>
